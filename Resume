<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Resume</title>
</head>
<body>
    <h1>
        SUHANI SIKARWAR
    </h1>
    <div>
        <h3>
            Junior Frontend Developer
        </h3>
        <p>
            8, Nai Basti Mursan
            <br>
            Mursan, Hathras (204213)
            <br>
            Contact No. - 9259573556
            <br>
            Email - suhanisikarwar35@gmail.com
        </p>
    </div>
    <div>
        <h3>
            Skills
        </h3>
        <p>
            HTML, CSS, Presentation, Video Editor, AutoCad user
        </p>
    </div>
    <div>
        <h3>
            Education
        </h3>
        <p>
            R.B.S Senior Secondary School, Mursan (Hathras)
            Compliting 12th from there 
            <br>
            I were performed no. of duties in my school life
            <br>
            I were House Captain and also Conduct the assembly including small events
            <br><br>
            Now, Pursuing BTECH(CSE) (2024-2028) from
            <br>
            GLA UNIVERSITY, Mathura

        </p>
    </div>
    <div>
        <h3>
            Experience
        </h3>
        <p>
            I am Fresher in this Field
        </p>
    </div>
    <div>
        <h3>
            Across the internet
        </h3>
        <p>
            <img src="data:image/png;base64,"  height="20px" text-decoration="none">

            <a href="https://www.linkedin.com/in/suhani-sikarwar-402686326?utm_source=share&utm_campaign=share_via&utm_content=profile&utm_medium=android_app" target ="_main">Linkedln </a>
            <br>
            
        </p>
    </div>
</body>
</html>
